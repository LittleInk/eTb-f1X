...
public class HUD extends Module {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        float lastWidth = event.getSR().getScaledWidth();
        ArrayList<Module> mods = new ArrayList(Client.INSTANCE.getModuleManager().getModuleMap().values());
        mods.sort(Comparator.comparingDouble(m -> -font.getStringWidth(m.getLabel() + (suffix.isEnabled() ? (m.getSuffix() != null ? ChatFormatting.GRAY + " - " + m.getSuffix() : "") : ""))));
        int y = 12;
        for (Module m : mods) {
            if (m.isEnabled() && !m.isHidden()) {
                ...
                float x = event.getSR().getScaledWidth() - font.getStringWidth(dispName) - 2;//event.getSR().getScaledWidth() * font.getStringWidth(dispName)/7;
    			float maxXValue = event.getSR().getScaledWidth() - font.getStringWidth(dispName);
    			if( x < maxXValue){
    				x = maxXValue;
    			}
    			//font.drawStringWithShadow("a" + x, event.getSR().getScaledWidth() - font.getStringWidth(dispName) - 2, y, rainbow.isEnabled() ? RenderUtil.getRainbow(3000, -(12 * y)) : m.getColor());
                
    			                RenderUtil.drawRect(x - 2,y,event.getSR().getScaledWidth() - 1, font.getStringHeight(dispName) - 0.8,
							new Color(0,0,0,120).getRGB());
                RenderUtil.drawRect(event.getSR().getScaledWidth() - 1,y,event.getSR().getScaledWidth() - 2, font.getStringHeight(dispName) - 0.8,
                		rainbow.isEnabled() ? RenderUtil.getRainbow(3000, -(12 * y)) : m.getColor());
                font.drawStringWithShadow(dispName, event.getSR().getScaledWidth() - font.getStringWidth(dispName) - 2, y, rainbow.isEnabled() ? RenderUtil.getRainbow(3000, -(12 * y)) : m.getColor());
